...
[Script]
Bili Region = type=http-response,pattern=^https:\/\/ap(p|i)\.bilibili\.com\/(pgc\/view\/(v\d\/)?app|x(\/v\d)?\/view\/video)\/(season|online)\?access_key,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/rickylsr/Rules/master/Surge/Surge%203/Bili_Auto_Regions.js
...
async function SwitchRegion() {
	...
	const TW = $.read('BiliArea_TW') || '台湾｜HINET｜CN2中转'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || '香港｜CN2'; //Your HongKong sub-policy name.
